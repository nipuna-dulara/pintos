...
int pintos_init(void)
{
  ...
  while (true)
  {
    ...
    printf("\r\n");
    if (!strcmp("whoami", input))
    {
      printf("nipuna-210467A ");
      printf("\r\n");
      continue;
    }
    if (!strcmp("shutdown", input))
    {
    ...
    if (!strcmp(input, "ram"))
    {
      printf("Available RAM for OS %u kB\n", init_ram_pages * PGSIZE / 1024);
      continue;
    }
    if (!strcmp(input, "thread"))
    {
      thread_print_stats();
      continue;
    }
    if (!strcmp(input, "priority"))
    {
      int prio = thread_get_priority();
      printf("priority of the active thread is %d\n", prio);
      continue;
    }
